...
fn bech_add(c: &mut Criterion) {
    c.bench_function("Adding 5k contact", |b| {
...
fn bench_list(c: &mut Criterion){
    ...
    c.bench_function("listing 5k contact", |b| {
        let mut storage = Store::new().expect("Store not created");
        storage.mem = storage.load().expect("Failed to load");
...
fn bench_search(c: &mut Criterion){
    ...
    c.bench_function("Searching 5k contact", |b| {
        let mut storage = Store::new().expect("Store not created");
        storage.mem = storage.load().expect("Failed to load");
...
fn bench_edit(c: &mut Criterion){
    ...
    c.bench_function("Editing 5k contact", |b| {
        let mut storage = Store::new().expect("Store not created");
        storage.mem = storage.load().expect("Failed to load");
        storage.mem.clear();
        let created_at = contact::Utc::now();
        
        storage.mem = (0..15_000)
            .map(|i| Contact {
                name: format!("User{i}"),
...
fn bench_delete(c: &mut Criterion){
    ...
    c.bench_function("Deleting 5k contact", |b| {
        let mut storage = Store::new().expect("Store not created");
        storage.mem.clear();
        let created_at = contact::Utc::now();
